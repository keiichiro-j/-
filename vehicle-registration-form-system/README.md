...
1. **専用スプレッドシートの作成**
   ...
   - テンプレートシート（`新車新規登録依頼書（書類送付書）OSS` / `新車新規登録依頼書（書類送付書）紙`）を用意する
     ...
     - 実物が無い場合: このプロジェクトに`SetupService.gs`を追加し、GASエディタの関数選択プルダウンから **`setupTemplateSheets`** を選んで一度だけ実行する。`Constants.gs`のセル位置定義から逆算して、ラベル・見出し・罫線付きの雛形シートを2つ自動生成する（見た目は生成後に手で調整してよい。再実行すると作り直されるので運用開始後は実行しないこと）
