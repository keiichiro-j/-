/**
 * HistoryService.gs
 * 履歴タブ(月次管理)の読み書きと、入力補助用サジェストの収集。
 * 新車新規登録依頼書システムと異なり、登録日は申請共通の1項目なので、
 * タブの振り分けも明細ごとではなく申請単位のregDateで決まる。
 */

function resolveHistoryTabName_(regDateStr) {
  if (!isValidDateStr_(regDateStr)) return HISTORY_PENDING_TAB_NAME;
  return formatYearMonth_(parseDateOnly_(regDateStr));
}
 ...
 * 既存タブでも、ヘッダーが現在のHISTORY_HEADER_ROWと一致しなければ上書きして揃える
 * (列追加などスキーマ変更前に作られたタブの場合。データ行そのものは書き換えない)。
 */
...
/**
 * 明細1件ぶんを、登録日が属する年月のタブへ1行追記する。
 * @param {string} pdfUrl この申請で発行されたPDFのURL
 */
function appendHistoryRow_(ss, docType, item, formData, submissionId, itemNo, timestamp, pdfUrl) {
  var tabName = resolveHistoryTabName_(formData.regDate);
  var sheet = getOrCreateHistoryTab_(ss, tabName);
  var isTransfer = docType === DOC_TYPE_TRANSFER;
  var isCancellation = docType === DOC_TYPE_CANCELLATION;

  var regClassValue = '';
  if (docType === DOC_TYPE_CERTIFICATE) regClassValue = item.regClass || '';
  else if (isTransfer) regClassValue = item.transferClass || '';
  else if (isCancellation) regClassValue = item.cancelClass || '';

  var variantParts = [];
  if (isTransfer && formData.hidaRegistration) variantParts.push('飛騨');
  if (isCancellation) variantParts.push(CANCELLATION_KIND_LABELS[formData.cancelKind] || CANCELLATION_KIND_LABELS[CANCELLATION_KIND_SIMPLE]);
  if ((isTransfer || isCancellation) && formData.isKei) variantParts.push('軽自動車');
  ...
  sheet.appendRow([
    ...
    formData.company,
    formData.manager,
    isValidDateStr_(formData.regDate) ? parseDateOnly_(formData.regDate) : '',
    ...
    regClassValue,
    item.correction || '',
    isTransfer || isCancellation ? toNonNegativeInt_(item.stamp) : (item.stamp || ''),
    isTransfer ? toNonNegativeInt_(item.plateFee) : '',
    isTransfer || isCancellation ? toNonNegativeInt_(item.agencyFee) : (item.agencyFee || ''),
    isTransfer ? toNonNegativeInt_(item.envTax) : '',
    isTransfer || isCancellation ? toNonNegativeInt_(item.keiFee) : '',
    docType === DOC_TYPE_PLATE_CHANGE ? (item.agencyFee || '') : '',
    item.remarks || '',
    isTransfer && formData.hidaRegistration ? '対象' : '',
    pdfUrl || '',
...
/**
 * "YYYY-MM"形式の月次タブ名のうち、直近monthsBack件を新しい順に返す(「登録日未定」タブも含む)。
 */
...
/**
 * 「登録日」がfromDate〜toDate(いずれも"YYYY-MM-DD"。空/不正なら片側無制限)の範囲に
...
/**
 * 指定したsubmissionIdを持つ全ての履歴行(複数明細・複数タブにまたがる場合を含む)を
...
function getPdfsBySendDateRange_(ss, fromDate, toDate, sendBatch) {
  ...
  var idx = {
    ...
    docType: header.indexOf('書類種別'),
    variant: header.indexOf('バリエーション'),
    company: header.indexOf('依頼会社名'),
    manager: header.indexOf('担当責任者'),
    sendDate: header.indexOf('送付日'),
    sendBatch: header.indexOf('送付便'),
    regNumber: header.indexOf('登録番号'),
    newRegNumber: header.indexOf('新登録番号'),
  ...
  getAllHistoryTabNames_(ss).forEach(function (name) {
    getHistoryEntries_(ss, name).rows.forEach(function (row) {
      ...
      bySubmission[id].itemCount++;
      var num = row[idx.regNumber] || row[idx.newRegNumber];
      if (num) bySubmission[id].regNumbers.push(num);
    });
  });
...
/**
 * 直近SUGGESTION_MONTHS_BACKヶ月分の履歴タブを横断して、依頼会社名・担当責任者の
 * サジェスト候補を収集する。
 */
function collectSuggestions_(ss) {
  ...
  var companyCol = HISTORY_HEADER_ROW.indexOf('依頼会社名');
  var managerCol = HISTORY_HEADER_ROW.indexOf('担当責任者');
  ...
  tabNames.forEach(function (name) {
    ...
    values.forEach(function (row) {
      if (row[companyCol]) companies[row[companyCol]] = true;
      if (row[managerCol]) managers[row[managerCol]] = true;
    });
  });
 ...
 * 直近1件の依頼会社名・送信時刻を返す(取消済みも件数に含める)。
