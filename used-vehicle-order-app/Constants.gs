 ...
 * ヘッダー・左レール自体は中立色（配色テーマの影響を受けない固定色）にしたため、
 * 以前あったsidebarColor（PC左端のサイドバー専用の背景色）は廃止した。
 */
var THEME_PRESETS = [
