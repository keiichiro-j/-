 ...
 *
 * Hold登録は、担当者本人がWebアプリを操作するその場で実行される
 ...
 * カレンダーへは書き込めない。そのため、期限切れによる自動解放時にはカレンダー
 * イベントの削除は行わない（期限が来て役目を終えたイベントが本人のカレンダーに
 * そのまま残るのみで、実害はない）。
 * 同様に、Hold中の車両を受注確定した場合に削除できるのは受注確定を行った本人
 * （＝Hold担当者。canConfirmOrder_によりHold担当者以外は受注確定できない）の
 * イベントのみ（README「Googleカレンダー連携」の項を参照）。
 */
