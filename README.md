...
## OCR抽出仕様（4.5 査定書 / 4.6 車検証）

OCRで読み取った文字列は、そのまま登録せず以下の業務ルールで正規化してから確認画面に表示します（`AppraisalExtractionService.gs` / `InspectionExtractionService.gs` / `OcrService.gs`）。

| 項目 | 抽出元 | 正規化ルール |
|---|---|---|
| OCN | - | 自動採番（既存最終番号+1）。OCRからは取得しない |
| 仕入日 | - | データ登録時の日付を自動設定 |
| 車種 | 査定書 | メーカー名を半角カタカナへ変換。メルセデス・ベンツ／アウディ／フォルクスワーゲンは MB/AU/VW と表記 |
| モデル | 査定書 | 独系（MB/AU）は「Aクラス180」→「A18」のようにクラス名+先頭2桁へ短縮。それ以外はそのまま |
| 車台番号 | 査定書 | 型式と区別し、車台番号ラベルの値をフルで取得 |
| 初年度登録 | 査定書 | 元号／西暦いずれの表記も `yyyy/MM` 形式に正規化 |
| 車検満了日 | 車検証 | 「有効期間の満了する日」等のラベルから `yyyy/MM/dd` 形式に正規化 |
| カラー | 査定書 | 色名を漢字一文字に正規化（例: ブラック→黒） |
| 走行距離 | 査定書 | `00,000km` 形式の文字列に整形 |
| 仕入区分 | 査定書 | 査定書から取り込んだ場合は既定値「下取」（確認画面で買取／仕入等へ変更可） |
| 担当者 | 査定書 | 査定担当者のフルネームを取得 |
| 仕入先・住所 | 車検証 | 使用者の氏名・住所を取得 |
| 登録番号 | 車検証 | 地域／分類番号／ひらがな／一連番号の4分割で抽出 |
| ステータス | - | データ反映時は「書類待ち」で固定登録。以後は書類待ち／車庫証明申請中／名義変更中／名義変更済み／抹消登録済み／保留／販売済みから選択 |
| 査定額・買取金額 | 査定書 | 査定書に記載の査定金額・買取金額をそのまま数値化 |
| 下取損 | - | 買取金額 － 査定額 で自動計算 |
| リサイクル金額 | 査定書 | リサイクル預託金をそのまま数値化 |

ラベル文字列・書式は査定書／車検証の実物サンプルに応じて `APPRAISAL_LABEL_MAP` / `INSPECTION_LABEL_MAP`（各Extractionサービスファイル冒頭）を調整してください。
