...
| `IntegratedViewService.gs` | 「3社統合ビュー」：AU/C7/MB 実運用スプレッドシート（輸入車マスタ／国産車マスタ／販売済み）を横断参照・編集。必須項目／OCN・車台番号重複チェック、LockServiceによる排他制御を追加 |
...
## 3社統合ビュー（AU/C7/MB 実データ）

既存の「3拠点横断」表示（1社1スプレッドシート・独自スキーマ）とは別に、AU/C7/MB の実運用スプレッドシート
（各社「輸入車マスタ」「国産車マスタ」「販売済み」の3シート）を直接横断参照・編集する画面を、
トップバーの「3社統合ビュー（実データ）」タブから利用できます。対象スプレッドシートIDと列位置は
`IntegratedViewService.gs` に定義済みです（コード内に直接記載。書き込み権限が必要な点に注意）。

- 一覧：3ファイル×3シートを横断集計。取得に失敗したファイルがあってもトーストで通知し、他ファイルの表示は継続
- 編集・新規登録：メーカー／モデル／車台番号は必須、OCN・車台番号は3ファイル横断で重複チェック
- 新規登録時のOCNは全ファイル中の最大値+1を自動採番（「輸入車マスタ」「国産車マスタ」のみ登録可）
- 保存はLockServiceで排他制御し、複数担当者の同時編集による競合・上書き事故を防止
- 編集時は保存直前にOCNを再照合し、他ユーザーによる削除・行ズレを検知してエラーにする
